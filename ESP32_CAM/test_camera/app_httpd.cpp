...
static esp_err_t index_handler(httpd_req_t *req) {
  ...
  page += "<button style=margin:5px;width:90px;height:70px;background-color:orange;color:white;font-weight:bold onmousedown=sendSerial('datax') ontouchstart=sendSerial('datax')>STOP</button>";
...
static esp_err_t dataT_handler(httpd_req_t *req) {
  Serial.println("t");
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, "OK", 2);
}
static esp_err_t dataW_handler(httpd_req_t *req) {
  Serial.println("w");
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, "OK", 2);
}
static esp_err_t dataA_handler(httpd_req_t *req) {
  Serial.println("a");
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, "OK", 2);
}
static esp_err_t dataS_handler(httpd_req_t *req) {
  Serial.println("s");
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, "OK", 2);
}
static esp_err_t dataD_handler(httpd_req_t *req) {
  Serial.println("d");
  httpd_resp_set_type(req, "text/html");
  return httpd_resp_send(req, "OK", 2);
}
...
void startCameraServer() {
  ...
  httpd_uri_t dataL_uri = {
    .uri       = "/datal",
    .method    = HTTP_GET,
    .handler   = dataL_handler,
    .user_ctx  = NULL
  };
    httpd_uri_t dataT_uri = {
    .uri       = "/datat",
    .method    = HTTP_GET,
    .handler   = dataT_handler,
    .user_ctx  = NULL
  };
    httpd_uri_t dataW_uri = {
    .uri       = "/dataw",
    .method    = HTTP_GET,
    .handler   = dataW_handler,
    .user_ctx  = NULL
  };
  httpd_uri_t dataA_uri = {
    .uri       = "/dataa",
    .method    = HTTP_GET,
    .handler   = dataA_handler,
    .user_ctx  = NULL
  };
  httpd_uri_t dataS_uri = {
    .uri       = "/datas",
    .method    = HTTP_GET,
